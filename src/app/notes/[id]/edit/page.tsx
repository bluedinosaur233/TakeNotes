...
export default async function EditNotePage({ params }: EditNotePageProps) {
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-1 flex-col px-6 py-12 lg:px-8">
      <Link href={`/notes/${note.id}`} className="text-sm font-semibold text-blue-600 hover:text-blue-700">
        ← 返回笔记详情
      </Link>
      <div className="mt-8 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-10">
        <p className="text-sm font-semibold text-blue-600">EDIT NOTE</p>
        <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-950">编辑学习笔记</h1>
        <p className="mt-3 leading-7 text-slate-600">修改内容后保存，更新时间会自动更新。</p>
        <div className="mt-8">
          <NoteForm
            action={updateNote}
